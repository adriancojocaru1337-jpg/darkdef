...
function computeMaxScore(wave, kills, bonus) {
...
exports.handler = async function handler(event) {
  ...
  try {
    ...
    if (run.mode !== "endless") {
      return reject({ statusCode: 400, error: "Only endless runs can be submitted", ipHash, playerName, runId, payload, suspicious: true, runDbId });
    }
    ...
    const maxScore = computeMaxScore(waveReached, killsCount, bonus);
    ...
    try {
      ...
      await sql`
        ...
        values
        (${playerName}, ${scoreTotal}, ${bonus}, ${waveReached}, ${killsCount}, 'endless', ${runId}, ${ipHash})
      `;
