...
for(let stage = 1; stage <= 6; stage++){
  ...
  stageGroundTextures[stage] = image;
}
const paintedPathTexture = new Image();
paintedPathTexture.decoding = "async";
paintedPathTexture.src = "assets/terrain/path.webp";
let paintedPathPattern = null;
paintedPathTexture.addEventListener("load", ()=>{
  paintedPathPattern = ctx.createPattern(paintedPathTexture, "repeat");
});
...
function enterEndlessModeFromUnlock(){
  ...
  stageWave = 1;
  wave = 1;
  // Endless starts its own score/bonus tally. The leaderboard ranks endless by
  // bonus_score and the server caps bonus against the endless wave count — if we
  // carried the campaign bonus over here it would exceed that cap the moment you
  // die early in endless, and the submission would be silently rejected.
  score = 0;
  bonusScore = 0;
  kills = 0;
  waveActive = false;
...
/* ===== Run Save / Resume =====
   ...
   stage transitions, on tab switch) so a closed tab never costs the player
...
function serializeRunState(){
  return {
    ...
    daily: dailyChallengeActive && activeDailyChallenge
      ...
      : null,
    leaderboardRun: leaderboardRun.runId ? { ...leaderboardRun } : null
  };
}
...
function restoreRunState(save){
  ...
  units.forEach(u => { u.snareTimer = 0; u.cooldown = 0; delete u.snaredUntil; });
...
// the resume overlay centered over the restored battlefield.
...
// Tab switch: snapshot the run (between waves) and auto-pause so nothing is
// lost while the player is away; they resume manually.
...
// Closing the page mid-build also snapshots the run.
window.addEventListener("beforeunload",()=>{
  if(isRunInProgress() && !waveActive && !pendingAuraChoice && !pendingBossResolution) saveRunState();
});
...
function drawPath(){
  ...
  if(paintedPathPattern){
    ...
    ctx.strokeStyle=paintedPathPattern;
    ctx.lineWidth=46;
...
startNewRunBtn?.addEventListener("click",()=>{ startNewRunFromResume(); });
